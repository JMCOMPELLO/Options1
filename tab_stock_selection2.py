...
class StockSelectionTab:
    ...
    def setup_ui(self):
        ...
        main_canvas = tk.Canvas(self.frame, bg=self.bg_main, highlightthickness=0)
        scrollbar = ttk.Scrollbar(self.frame, orient="vertical", command=main_canvas.yview)
        scrollable_frame = tk.Frame(main_canvas, bg=self.bg_main)
        ...
        scrollable_frame.bind('<Enter>', lambda e: main_canvas.bind_all("<MouseWheel>", _on_mousewheel))
        scrollable_frame.bind('<Leave>', lambda e: main_canvas.unbind_all("<MouseWheel>"))
        
        # Header
        header = tk.Frame(scrollable_frame, bg=self.bg_dark, height=80)
        ...
        # Info panel
        info_panel = tk.LabelFrame(main_frame, text=" 📌 STEP 1: SELECT UNDERLYING ASSETS ", 
                                   bg="#e8f5e9", fg="black", font=("Arial", 13, "bold"), bd=3)
        ...
        tk.Label(info_panel, text="Select stocks to backtest options strategies on. "
                ...
                bg="#e8f5e9", fg="#2e7d32", font=("Arial", 11), wraplength=1200,
                justify=tk.LEFT).pack(padx=15, pady=10)
        
        # Bulk selection panel
        bulk_panel = tk.LabelFrame(main_frame, text=" BULK SELECTION ", 
                                   bg="#f5f5f5", fg="black", font=("Arial", 12, "bold"), bd=2)
        bulk_panel.pack(fill=tk.X, pady=(0, 10))
        
        btn_frame = tk.Frame(bulk_panel, bg="#f5f5f5")
        btn_frame.pack(fill=tk.X, padx=15, pady=10)
        
        tk.Button(btn_frame, text="✓ SELECT ALL S&P 500", bg=self.accent, fg="black",
        ...
        tk.Button(btn_frame, text="💰 FINANCIAL", bg="#FF9800", fg="white",
        ...
        # Search
        search_frame = tk.Frame(main_frame, bg=self.bg_main)
        search_frame.pack(fill=tk.X, pady=(0, 10))
        
        tk.Label(search_frame, text="🔍 SEARCH:", fg="black", bg=self.bg_main, 
        ...
        selected_panel = tk.LabelFrame(main_frame, text=" SELECTED PORTFOLIO ", 
                                       bg="#fff3e0", fg="black", font=("Arial", 12, "bold"), bd=2)
        selected_panel.pack(fill=tk.X, pady=(0, 10))
        
        self.selected_text = tk.Text(selected_panel, height=4, wrap=tk.WORD, 
                                     font=("Arial", 11, "bold"), bg="#fffef7", fg="#000000",
        ...
        # Footer
        footer = tk.Frame(scrollable_frame, bg="#f5f5f5", height=100)
        footer.pack(fill=tk.X, padx=30, pady=20)
        ...
        next_btn = tk.Button(footer, text="NEXT: CONFIGURE OPTIONS STRATEGY →", 
                            bg=self.accent, fg="black", font=("Arial", 16, "bold"), 
    ...
    def create_sector_checkboxes(self):
        ...
        for idx, sector in enumerate(sectors):
            ...
            cb = tk.Checkbutton(self.sector_checkbox_frame, text=sector, variable=var,
                          command=self.on_sector_toggle, bg="#f5f5f5", fg="black",
