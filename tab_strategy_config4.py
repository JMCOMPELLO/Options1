...
class StrategyConfigTab:
    ...
    def setup_ui(self):
        # Main container - NO CANVAS, direct scrolling
        main_container = tk.Frame(self.frame, bg="#f0f0f0")
        main_container.pack(fill=tk.BOTH, expand=True)
        
        # Header
        header = tk.Frame(main_container, bg="#1a1a1a", height=90)
        ...
        # Scrollable content with canvas (but simpler)
        ...
        # Better Mac scrolling - bind to the canvas and all children
        def _on_scroll(event):
            canvas.yview_scroll(-1 * int(event.delta), "units")
        
        # Bind to canvas and recursively to all children
